...
//            listBoxCompanies.SelectedIndexChanged += listBoxCompanies_SelectedIndexChanged;
        ...
        private void DoDaFix()
        {
            ...
            string originatingAtty = "";
            string firstOrigAtty = "";
            var selectedItems = listView1.SelectedItems;
            if (selectedItems.Count > 0)
            {
                foreach (ListViewItem selectedItem in selectedItems)
                {
                    originatingAtty = originatingAtty + selectedItem.SubItems[0].Text + "','";
                    if (string.IsNullOrEmpty(firstOrigAtty))
                        firstOrigAtty = selectedItem.SubItems[0].Text;
                }
                originatingAtty = originatingAtty.Remove(originatingAtty.Length -3, 3);
                ...
                if (string.IsNullOrEmpty(originalDB))
                    ...
                    runSQLqueries(originatingAtty, firstOrigAtty, originalDB);
            }
            else
        ...
        private void runSQLqueries(string origAtty, string firstAtty, string origDB)
        {
            ...
            try
            {
                ...
                errorTable = "FeeSched";
                commandSQL = "Insert into " + JurisDbName + ".dbo.FeeSchedule(FeeSchCode,FeeSchDesc,FeeSchActive) Select FeeSchCode,FeeSchDesc,FeeSchActive from " + origDB + ".dbo.FeeSchedule where exists (select matfeesch from " + origDB + ".dbo.matter inner join " + origDB + ".dbo.matorigatty on matsysnbr=morigmat inner join " + origDB + ".dbo.employee on employee.empsysnbr=morigatty where " + origDB + ".dbo.employee.EmpID in ('" + origAtty + "'))" +
                    " or exists  (Select FeeSchCode,FeeSchDesc,FeeSchActive from " + origDB + ".dbo.FeeSchedule where exists (select clifeesch from " + origDB + ".dbo.client inner join " + origDB + ".dbo.cliorigatty on corigcli=clisysnbr inner join " + origDB + ".dbo.employee on employee.empsysnbr=corigatty where exists (select empid from " + origDB + ".dbo.employee where EmpID in ('" + origAtty + "')))) or " + origDB + ".dbo.FeeSChedule.feeschcode='STDR'" +
                    " Insert into " + JurisDbName + ".dbo.DocumentTree(dtdocid,DTSystemCreated,DTDocClass,DTDocType,DTParentID,DTTitle,DTKeyT) " +
                ...
                errorTable = "ExpSched";
                commandSQL = " Insert into " + JurisDbName + ".dbo.ExpenseSchedule(ExpSchCode,ExpSchDesc) Select ExpSchCode,ExpSchDesc from " + origDB + ".dbo.ExpenseSchedule where  expschcode in (select matexpsch from " + origDB + ".dbo.matter inner join " + origDB + ".dbo.matorigatty on matsysnbr=morigmat inner join " + origDB + ".dbo.employee on employee.empsysnbr=morigatty where " + origDB + ".dbo.employee.EmpID in ('" + origAtty + "'))" +
                    " or expschcode in (select cliexpsch from " + origDB + ".dbo.client inner join " + origDB + ".dbo.cliorigatty on corigcli=clisysnbr inner join " + origDB + ".dbo.employee on empsysnbr=corigatty where exists (select empid from " + origDB + ".dbo.employee where EmpID  in ('" + origAtty + "'))) or expschcode='STDR'";
                _jurisUtility.ExecuteNonQueryCommand(0, commandSQL);
                ...
                commandSQL = " Insert into " + JurisDbName + ".dbo.Employee(EmpSysNbr,EmpID,EmpInitials,EmpPassword,EmpValidAsUser,EmpValidAsTkpr,EmpMnuPerm,EmpRptPerm,EmpOnLine,EmpPreferences,EmpPrsTyp,EmpSortSeq,EmpFeeTaxExempt,EmpBudHrsDay,EmpBudTargetRate,EmpNetID,EmpFirstName,EmpMiddleName,EmpLastName,EmpEmail) " +
                    ...
                    "  where empsysnbr<>1 and (empsysnbr in (select tbdtkpr from " + origDB + ".dbo.timebatchdetail inner join  " + origDB + ".dbo.matter  on tbdmatter=matsysnbr where MatSysNbr " +
                    "  in (select MatSysNbr from " + origDB + ".dbo.matter inner join " + origDB + ".dbo.billto on matbillto=billtosysnbr inner join " + origDB + ".dbo.employee on empsysnbr=billtobillingatty where billtobillingatty in (select EmpSysNbr from " + origDB + ".dbo.employee where empid in ('" + origAtty + "')))))" +
                    " or empsysnbr in (Select clibillingatty from " + origDB + ".dbo.client inner join " + origDB + ".dbo.employee on employee.empsysnbr=client.clibillingatty where EmpID in ('" + origAtty + "'))" +
                    " or empsysnbr in (select billtobillingatty from " + origDB + ".dbo.billto inner join " + origDB + ".dbo.matter on matbillto=billtosysnbr inner join " + origDB + ".dbo.employee on empsysnbr=billtobillingatty where billtobillingatty in (select EmpSysNbr from " + origDB + ".dbo.employee where empid in ('" + origAtty + "')))" +
                    "  or empsysnbr in (select morigatty from " + origDB + ".dbo.matorigatty inner join " + origDB + ".dbo.matter on morigmat=matsysnbr inner join " + origDB + ".dbo.employee on empsysnbr=morigatty where EmpID in ('" + origAtty + "'))" +
                    "  or empsysnbr in (select corigatty from " + origDB + ".dbo.cliorigatty inner join " + origDB + ".dbo.client on corigcli=clisysnbr inner join " + origDB + ".dbo.employee on empsysnbr=corigatty where EmpID in ('" + origAtty + "'))" +
                    "  or empsysnbr in (select MRTEmployeeID from " + origDB + ".dbo.MatterResponsibleTimekeeper inner join " + origDB + ".dbo.matter on MRTMatterID=matsysnbr inner join " + origDB + ".dbo.employee on empsysnbr=mrtemployeeid where EmpID in ('" + origAtty + "'))" +
                    "  or empsysnbr in (select CRTEmployeeID from " + origDB + ".dbo.ClientResponsibleTimekeeper inner join " + origDB + ".dbo.client on CRTClientID=clisysnbr inner join " + origDB + ".dbo.employee on empsysnbr=crtemployeeid where EmpID in ('" + origAtty + "'))";
                ...
                commandSQL = " Insert into " + JurisDbName + ".dbo.Client(CliSysnbr,CliCode,CliNickName,CliReportingName,CliSourceOfBusiness,CliPhoneNbr,CliFaxNbr,CliContactName,CliDateOpened,Cliofficecode,CliBillingAtty,CliPracticeClass,CliFeeSch,CliTaskCodeXref,CliExpSch,CliExpCodeXref,CliBillFormat,CliBillAgreeCode,CliFlatFeeIncExp,CliRetainerType,CliExpFreqCode,CliFeeFreqCode,CliBillMonth,CliBillCycle,CliExpThreshold,CliFeeThreshold,CliInterestPcnt,CliInterestDays,CliDiscountOption,CliDiscountPcnt,CliSurchargeOption,CliSurchargePcnt,CliTax1Exempt,CliTax2Exempt,CliTax3Exempt,CliBudgetOption,CliReqPhaseOnTrans,CliReqTaskCdOnTime,CliReqActyCdOnTime,CliReqTaskCdOnExp,CliPrimaryAddr,CliType,CliEditFormat,CliThresholdOption,CliRespAtty,CliBillingField01,CliBillingField02,CliBillingField03,CliBillingField04,CliBillingField05,CliBillingField06,CliBillingField07,CliBillingField08,CliBillingField09,CliBillingField10,CliBillingField11,CliBillingField12,CliBillingField13,CliBillingField14,CliBillingField15,CliBillingField16,CliBillingField17,CliBillingField18,CliBillingField19,CliBillingField20,CliCTerms,CliCStatus,CliCStatus2) " +
                    ...
                    " where " + origDB + ".dbo.Client.clicode in (select clicode from " + origDB + ".dbo.client inner join " + origDB + ".dbo.cliorigatty on corigcli=clisysnbr where COrigAtty in (select empsysnbr from " + origDB + ".dbo.employee where empid in ('" + origAtty + "'))) or " + origDB + ".dbo.Client.clisysnbr in (select matclinbr from " + origDB + ".dbo.matter inner join " + origDB + ".dbo.matorigatty on matsysnbr=morigmat inner join " + origDB + ".dbo.employee on employee.empsysnbr=morigatty where morigatty in (select EmpSysNbr from " + origDB + ".dbo.employee where EmpID in ('" + origAtty + "')))";
                ...
                //there may be a situation where an attorney who wasnt selected as originating needs to be removed and reassigned to
                //a proper originating attorney. If so, we need to assign the percentage to the new originating attorney
                while (true)
                {
                    string SQLCO = @"select cast(corigcli as varchar(10)) as Cli, cast(corigatty as varchar(10)) as Corig, cast(corigpcnt as varchar(10)) as Pct from " + JurisDbName + ".dbo.cliorigatty where COrigAtty not in (select empsysnbr from " + JurisDbName + ".dbo.employee where empid in ('" + origAtty + "'))";
                    DataSet myRSCMQ = _jurisUtility.RecordsetFromSQL(SQLCO);
                    if (myRSCMQ.Tables[0].Rows.Count != 0)
                    {
                        string Cli = myRSCMQ.Tables[0].Rows[0]["Cli"].ToString();
                        string OAtty = myRSCMQ.Tables[0].Rows[0]["Corig"].ToString();
                        string Pct = myRSCMQ.Tables[0].Rows[0]["Pct"].ToString();


                        //remove the offending row
                        string CC = "delete from " + JurisDbName + ".dbo.cliorigatty  where corigcli=cast('" + Cli + "' as int) and corigatty=cast('" + OAtty + "' as int)";
                        _jurisUtility.ExecuteNonQueryCommand(0, CC).ToString();

                        //if there is another row for this client....
                        if (Convert.ToDouble(Pct) < 100)
                        {
                            string CC2 = "update " + JurisDbName + ".dbo.cliorigatty set corigpcnt = corigpcnt + cast('" + Pct.ToString() + "' as decimal(7,4))  where corigcli=(select top 1 cast(corigcli as int) from " + JurisDbName + ".dbo.cliorigatty where corigcli=cast('" + Cli + "' as int)) ";
                            _jurisUtility.ExecuteNonQueryCommand(0, CC2).ToString();
                        }
                        else
                        {
                            string CC2 = "Insert into " + JurisDbName + ".dbo.CliOrigAtty(COrigCli,COrigAtty,COrigPcnt) values(cast('" + Cli + "' as int), (select top 1 EmpSysNbr from " + JurisDbName + ".dbo.employee where EmpID in ('" + origAtty + "')), 100.0000 )";
                            _jurisUtility.ExecuteNonQueryCommand(0, CC2).ToString();
                        }
                    }
                    else
                ...
                commandSQL = " Insert into " + JurisDbName + ".dbo.BillCopy(BilCpyBillTo,BilCpyBilAdr,BilCpyComment,BilCpyNbrOfCopies,BilCpyPrintFormat,BilCpyEmailFormat,BilCpyExportFormat,BilCpyARFormat) " +
                    ...
                    " where " + origDB + ".dbo.billcopy.bilcpybillto in (select matbillto from " + origDB + ".dbo.matter inner join " + origDB + ".dbo.matorigatty on matsysnbr=morigmat inner join " + origDB + ".dbo.employee on empsysnbr=morigatty where EmpID in ('" + origAtty + "'))";
                _jurisUtility.ExecuteNonQueryCommand(0, commandSQL);
                ...
                commandSQL = " Insert into " + JurisDbName + ".dbo.Matter(MatSysNbr,MatCliNbr,MatBillTo,MatCode,MatNickName,MatReportingName,MatDescription,MatRemarks,MatPhoneNbr,MatFaxNbr,MatContactName,MatDateOpened,MatStatusFlag,MatLockFlag,MatDateClosed,MatOfficeCode,MatPracticeClass,MatFeeSch,MatTaskCodeXref,MatExpSch,MatExpCodeXref,MatQuickAction,MatBillAgreeCode,MatFlatFeeIncExp,MatRetainerType,MatFltFeeOrRetainer,MatExpFreqCode,MatFeeFreqCode,MatBillMonth,MatBillCycle,MatExpThreshold,MatFeeThreshold,MatInterestPcnt,MatInterestDays,MatDiscountOption,MatDiscountPcnt,MatSurchargeOption,MatSurchargePcnt,MatSplitMethod,MatSplitThreshold,MatSplitPriorAmtBld,MatBudgetOption,MatBudgetPhase,MatReqPhaseOnTrans,MatReqTaskCdOnTime,MatReqActyCdOnTime,MatReqTaskCdOnExp,MatTax1Exempt,MatTax2Exempt,MatTax3Exempt,MatDateLastWork,MatDateLastExp,MatDateLastBill,MatDateLastStmt,MatDateLastPaymt,MatLastPaymtAmt,MatARLastBill,MatPaySinceLastBill,MatAdjSinceLastBill,MatPPDBalance,MatVisionAddr,MatThresholdOption,MatType,MatBillingField01,MatBillingField02,MatBillingField03,MatBillingField04,MatBillingField05,MatBillingField06,MatBillingField07,MatBillingField08,MatBillingField09,MatBillingField10,MatBillingField11,MatBillingField12,MatBillingField13,MatBillingField14,MatBillingField15,MatBillingField16,MatBillingField17,MatBillingField18,MatBillingField19,MatBillingField20,MatCTerms,MatCStatus,MatCStatus2) " +
                    ...
                    "  inner join " + origDB + ".dbo.employee on employee.empsysnbr=billto.billtobillingatty where " + origDB + ".dbo.employee.EmpID in ('" + origAtty + "')";
                ...
                _jurisUtility.ExecuteNonQueryCommand(0, commandSQL);
                UpdateStatus("Updating Database", 52, steps);
        ...
        private void listBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (listBoxSelected)
            {
                ...
                if (isJurisDB)
                {
                    ...
                    SqlDataAdapter da = new SqlDataAdapter();
                    SqlCommand cmd = conn.CreateCommand();
                    cmd.CommandText = "select EmpID, EmpName from Employee";
                    da.SelectCommand = cmd;
                    DataSet ds = new DataSet();
                    ...
                    listView1.Columns.Add("Attorney Code", 85);
                    listView1.Columns.Add("Attorney Name", 300);
        ...
        private void buttonSelect_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Could not connect to SQL Server. Details: " + ex4.Message);
            }
        }
